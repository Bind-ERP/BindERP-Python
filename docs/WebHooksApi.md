...
new_web_hook_subscription = swagger_client.NewWebHookSubscription() # NewWebHookSubscription | 

try:
    # Suscribirse a un WebHook
    api_response = api_instance.web_hooks_add_web_hook_subscriptions(new_web_hook_subscription)
    pprint(api_response)
...
try:
    ...
    print("Exception when calling WebHooksApi->web_hooks_delete_web_hook_subscription_by_id: %s\n" % e)
```
...
# create an instance of the API class
api_instance = swagger_client.WebHooksApi()
web_hook_subscription = swagger_client.EditWebHookSubscription() # EditWebHookSubscription | 
